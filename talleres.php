...
$talleres = $conexion->query("SELECT * FROM talleres WHERE tipo = 'taller' ORDER BY id DESC");
$disciplinas = $conexion->query("SELECT * FROM talleres WHERE tipo = 'disciplina' ORDER BY id DESC");
...
<head>
  ...
  <title>Talleres | Técnica 109</title>
...
<div class="container contenedor-talleres">
  <h2 class="text-center text-uppercase mb-4" style="color: #800020;">Talleres y Disciplinas</h2>
  <div class="row">
    <!-- Columna de Talleres -->
    <div class="col-md-6">
      <h4 class="text-center text-decoration-underline mb-3">Talleres</h4>
      <?php while ($fila = $talleres->fetch_assoc()): ?>
        <div class="card mb-4">
          <img src="<?= htmlspecialchars($fila['imagen']) ?>" class="card-img-top" alt="<?= htmlspecialchars($fila['titulo']) ?>" style="max-height: 300px; object-fit: contain; width: 100%; background-color: #f9f9f9; padding: 10px;">
          <div class="card-body">
            <h5 class="card-title"><?= htmlspecialchars($fila['titulo']) ?></h5>
            <p class="card-text"><?= nl2br(htmlspecialchars($fila['descripcion'])) ?></p>
          </div>
        </div>
      <?php endwhile; ?>
    </div>
    ...
    <div class="col-md-6">
      <h4 class="text-center text-decoration-underline mb-3">Disciplinas</h4>
      <?php while ($fila = $disciplinas->fetch_assoc()): ?>
        <div class="card mb-4">
         <img src="<?= htmlspecialchars($fila['imagen']) ?>" class="card-img-top" alt="<?= htmlspecialchars($fila['titulo']) ?>" style="max-height: 300px; object-fit: contain; width: 100%; background-color: #f9f9f9; padding: 10px;">
          <div class="card-body">
            <h5 class="card-title"><?= htmlspecialchars($fila['titulo']) ?></h5>
